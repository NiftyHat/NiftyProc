...
namespace NiftyProc
{
    ...
    public static class UniformPoissonDiskSampler
    {
        ...
        {
            return Sample(center - new Vector2(radius,0), center + new Vector2(radius,0), radius, minimumDistance,
                pointsPerIteration);
        }
        
        public static List<Vector2> SampleRectangle(Godot.Rect2 rect2, float minimumDistance)
        {
            return SampleRectangle(rect2.Position, rect2.End, minimumDistance);
        }
        ...
        static bool AddNextPoint(Vector2 point, ref Settings settings, ref State state)
        {
            ...
            if (q.x >= settings.TopLeft.x && q.x < settings.LowerRight.x &&
                ...
                (settings.RejectionSqDistance == null ||
                 settings.Center.DistanceSquaredTo(q) <= settings.RejectionSqDistance))
            {
                ...
                for (var j = (int)Math.Max(0, qIndex.y - 2);
                    ...
                    if (state.Grid[i, j].HasValue &&
                        state.Grid[i, j].Value.DistanceTo(q) < settings.MinimumDistance)
                        tooClose = true;
